...
module "static-website" {
    source = "github.com/pbs/terraform-static-website-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/static-website'
gh -R 'pbs/terraform-static-website-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/static-website'
rm -f release.tar.gz
```
...
```bash
git remote add -f static-website git@github.com:pbs/terraform-static-website-module.git
git subtree add --prefix terraform/modules/static-website static-website main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/static-website static-website $REF --squash
```
